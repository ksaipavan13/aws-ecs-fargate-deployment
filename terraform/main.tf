...
# ECR Repositories (if needed, but now we use Docker Hub image for Strapi)
resource "aws_ecr_repository" "reactjs" {
  name = "reactjs"
}
...
resource "aws_ecs_task_definition" "reactjs" {
  ...
  container_definitions = jsonencode([{
    ...
    image = "533266978173.dkr.ecr.ca-central-1.amazonaws.com/reactjs:latest"
    portMappings = [{
      containerPort = 80
      hostPort      = 80
    }]
